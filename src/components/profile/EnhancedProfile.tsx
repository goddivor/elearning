...
import { Edit2, Save2, CloseSquare, Lock, Camera, Location, Link2, Add, Trash } from 'iconsax-react';
import { FacebookLogo, InstagramLogo, TwitterLogo, LinkedinLogo, YoutubeLogo, GithubLogo, TelegramLogo, WhatsappLogo, DiscordLogo } from '@phosphor-icons/react';
...
interface EnhancedProfileProps {
  roleLabel: string;
  roleColor: string;
  roleIcon: React.ComponentType<IconProps>;
  bannerGradient: string;
}

const EnhancedProfile = ({ roleLabel, roleColor, roleIcon: RoleIcon, bannerGradient }: EnhancedProfileProps) => {
  ...
  // Fonction pour détecter l'icône selon l'URL
  const getSocialIcon = (url: string) => {
    const lowerUrl = url.toLowerCase();
    if (lowerUrl.includes('facebook.com')) return <FacebookLogo size={20} weight="fill" color="#1877F2" />;
    if (lowerUrl.includes('instagram.com')) return <InstagramLogo size={20} weight="fill" color="#E4405F" />;
    if (lowerUrl.includes('twitter.com') || lowerUrl.includes('x.com')) return <TwitterLogo size={20} weight="fill" color="#1DA1F2" />;
    if (lowerUrl.includes('linkedin.com')) return <LinkedinLogo size={20} weight="fill" color="#0A66C2" />;
    if (lowerUrl.includes('youtube.com')) return <YoutubeLogo size={20} weight="fill" color="#FF0000" />;
    if (lowerUrl.includes('github.com')) return <GithubLogo size={20} weight="fill" color="#181717" />;
    if (lowerUrl.includes('t.me') || lowerUrl.includes('telegram')) return <TelegramLogo size={20} weight="fill" color="#26A5E4" />;
    if (lowerUrl.includes('wa.me') || lowerUrl.includes('whatsapp')) return <WhatsappLogo size={20} weight="fill" color="#25D366" />;
    if (lowerUrl.includes('discord')) return <DiscordLogo size={20} weight="fill" color="#5865F2" />;
    return <Link2 size={18} color="#6B7280" />;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {/* Banner - More space */}
        <div
          className={`relative h-72 ${bannerGradient} overflow-hidden group`}
          ...
          <div className="flex items-end justify-between mb-6">
            <div className="flex items-end gap-6">
              ...
              <div className="pb-4">
                ...
                <p className={`${roleColor} font-medium flex items-center gap-2 mt-2`}>
                  <RoleIcon size={18} color="currentColor" />
                  {roleLabel}
                </p>
                {formData.location && (
      ...
      {/* Social Links Section - Separate Card */}
